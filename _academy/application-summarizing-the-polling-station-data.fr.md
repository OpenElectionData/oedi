...
La première ligne du fichier contient le nom des variables. Il y a six variables dans cette série de données: «PS\_Code» (Code\_BV), «PS\_Name» (Nom\_BV), «Region\_Name» (Nom\_Région), «District\_Name» (Nom\_District), «Constituency\_Name» (Nom\_Circonscription) et «Registered\_Voters» (Electeurs\_Inscrits).
...
Il y a 20 928 observations pour la variable appelée «Registered\_Voters» (Electeurs\_Inscrits). Ensuite, nous allons nous concentrer sur cette variable de «Registered\_Voters» (Electeurs\_Inscrits).

### Vérifier vos Attentes: la variable «Registered\_Voters» (Electeurs\_Inscrits)

La Colonne F contient le nombre d'électeurs inscrits pour chaque bureau de vote (c.-à-d les ligne). Avant de synthétiser la variable «Registered\_Voters»(Electeurs\_Inscrits), prenons un moment pour réfléchir à ce que nous nous attendons à voir. Posez-vous quelques questions, telles que:
...
Maintenant, vous serez en mesure de sélectionner la plage que vous voulez décrire. Nous sommes intéressés par la variable «Registered\_Voters» (Electeurs\_Inscrits) qui est dans la colonne F. Donc, nous sélectionnons le haut de la colonne F pour sélectionner toute la colonne.
...
Ensuite, cliquez pour revenir sur cette case à droite de la plage d'entrée (![Input box](/images/academy/application-summarizing-the-polling-station-data/input_box.png)).
...
Maintenant, il va vous montrer le nouvel onglet avec toutes les statistiques sommaires pour «Registered\_Voters.» (Electeurs\_Inscrits)
...
Compte tenu de ce que vous savez maintenant sur la moyenne, l'écart-type et l'étendue, comment décririez-vous la variable «Registered\_Voters» (Electeurs\_Inscrits)? Pensez-vous que la distribution est bien dispersée ou pensez-vous qu'elle est bien regroupée?

Comparer les Statistiques Réelles aux Attentes
----------------------------------------------

Maintenant que vous avez les statistiques sommaires pour la variable des «Registered\_Voters» (Electeurs\_Inscrits), comment voulez-vous répondre aux questions suivantes? Comment sont les réponses par rapport aux attentes que vous aviez au début?
