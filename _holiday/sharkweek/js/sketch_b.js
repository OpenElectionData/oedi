...
function setup() {
    ...
    if (livesEarned > 0) {
      ...
      if (
        document.cookie.replace(
          /(?:(?:^|.*;\s*)quizLivesEarned\s*\=\s*([^;]*).*$)|^.*$/,
          '$1'
        ) <=
        numQuestions * 2
      ) {
        lives = document.cookie.replace(
          /(?:(?:^|.*;\s*)quizLivesEarned\s*\=\s*([^;]*).*$)|^.*$/,
          '$1'
        )
  ...
  $('#play-game').click(function() {
    ...
    // Set our lives to 1; this is a teaser
    lives = 1
...
function draw() {
  ...
  if (gameStarted == true) {
    ...
    if (freePlayModeFlag == true && lives <= 0) {
      ...
      freePlayModeFlag == false &&
      (lives <= 0 || numQuestionsIncorrect == numQuestions)
    ) {
      gameOver()
    }
  } else {
    ...
    if (freePlayModeFlag == false) {
      if (score > 0) {
        $('#game-over-results')
          .show()
          .html(
            "<span class='right'>FIN-tastic!</span><br />You earned <strong>" +
              score +
              ' lives</strong> to help you and OEDI collect the missing ballots in our free play game! Check back tomorrow for new questions!'
          )

        // Show the share buttons
        shareButtons(
          'I earned ' + score + ' extra lives in Ballots: Casted Away!'
        )
        ...
        $('#game-over-results')
          .show()
          .html(
            "<span class='wrong'>Uh-oh!</span><br />It looks like that quiz took a byte out of you.<br />Give it another try to unlock the free play version of the game!"
          )
      }
      ...
      $('#game-over-results')
        .show()
        .html(
          '<span>FIN-tastic!</span><br />You saved <strong>' +
            score +
            ' missing ballots</strong>!<br />To keep playing, earn more lives by taking our quiz on open election data!'
        )
      ...
      // Show the share buttons
      shareButtons(
        'I rescued ' + score + ' missing ballots in Ballots: Casted Away!'
      )
    } else {
      ...
      if (freePlayCounter > 0) {
        $('#game-over-results')
          .show()
          .html(
            '<span>FIN-tastic!</span><br />You collected <strong>' +
              score +
              ' missing ballots</strong>! To play again, take our quiz or sign in! Be sure to check back tomorrow for new questions!'
          )
      } else {
        // Personal High Score
        var bonus = ''
        ...
        if (score > highScore) {
          bonus =
            'JAWesome! You have set the record with the highest score so far!'
        }
        ...
        $('#game-over-results')
          ...
          .html(
            '<span>FIN-tastic!</span><br />You saved <strong>' +
              score +
              ' missing ballots</strong>!<br />' +
              bonus +
              '<br />Check back tomorrow for new quiz questions!'
          )
      }
...
Shark.prototype.display = function() {
  ...
  switch (this.type) {
    case 1:
      image(sharkRed, this.xpos, this.ypos, 46, 100)
      break
    case 2:
      image(sharkGreen, this.xpos, this.ypos, 46, 100)
      break
    case 3:
      image(sharkOrange, this.xpos, this.ypos, 46, 100)
      break
    case 4:
      image(sharkBlue, this.xpos, this.ypos, 46, 100)
      break
  }
...
$('.btn-instructions').click(function() {
  if (
    ($('#game_instructions_modal').data('bs.modal') || {}).isShown ==
    (false || undefined)
  ) {
    pauseGame()
  }
