  ...
  (function() {
    ...
    function selectCharacter(
      ...
      if (characterSelected != '') {
        var returnTopDiff =
          $('section#opening .oedi').offset().top -
          $('.' + characterSelected).offset().top +
          characterTopOffset
        var returnLeftDiff =
          $('section#opening .oedi').offset().left -
          $('.' + characterSelected).offset().left +
          characterLeftOffset
    ...
    $('#play').click(function() {
      if (!characterSelected) {
        $('.char-error').html(
          'You must select a character before playing the game.'
        )
        return
      }
      ...
      $('#opening').fadeOut('slow', function() {
        ...
        body.classList.remove('starry-bg')
    ...
    function checkAnswer(question) {
      ...
      //If Wrong Answer
        ...
        if (tries == 3) {
          ...
          $('.quiz_container .result')
            .addClass('wrong')
            .html(incorrectResponseArray[tries - 1])
    ...
    function oediWan() {
      ...
      $('#oedi-wan').fadeIn('slow', function() {
        $('.quiz_container .result')
          .addClass('right')
          .html(
            'OEDI-wan Kanobi has highlighted the correct answer! Click submit to go to the next question.'
          )
    ...
    function correctAnswer() {
      ...
      $('.' + characterSelected + ' .shot' + questionNumber).transition(
        { x: '600px' },
        500,
        'linear',
        function() {
          ...
          $('.g' + questionNumber + ' .chain1')
            .transition({ y: '100px' }, 500)
            .hide('slow')
          $('.g' + questionNumber + ' .chain2')
            .transition({ y: '100px' }, 500)
            .hide('slow')
          $('.g' + questionNumber + ' .lock')
            .transition({ y: '100px' }, 500)
            .hide('slow')

          // Move the Freed Principles & Add Freed Class
          $('.g' + questionNumber).transition({
            x: principlesLeftDistance,
            delay: 600
          })
      ...
      $('.jabba_container').transition(
        ...
        function() {
          ...
          $('#oedi-wan').fadeIn('slow', function() {
            ...
            $('.quiz_container .result')
              .addClass('right')
              .addClass('congratulations')
              .html(
                'Well done!  In my experience there is no such thing as luck.'
              )
          })
          // Play the Music
          $('#music_container').html(
            "<audio id='cantinaMusic' src='dw_music/cantina_cut.ogg' controls muted='true'/>"
          )
